...
the registry entry is updated accordingly; if the target no longer exists, the stale
entry is discarded and resolution falls through to the next step.
\item On a registry miss (or a discarded stale entry), the target provider is queried
...
The four objectives stated in Section~\ref{sec:int_objectives} were met:
\begin{enumerate}
  ...
  endpoints.
  \item \textbf{Extend QuickFaaS to AWS Lambda} --- QuickFaaS gained an AWS provider and OmniFlow an
  \texttt{AwsLambdaDeployer} (Sections~\ref{subsec:quickfaas-aws-provider}
  and~\ref{subsec:omniflow-aws-deployer}), so the unification is available on both AWS and GCP,
  and the resolver tests cover both providers (Section~\ref{sec:eval-correctness}).
...
through an explicit resolution cascade. The overhead of this unification is small and, with the
single-read optimization now applied uniformly across both production resolvers, asymptotically
linear. What remains is engineering maturity --- registry integrity, least-privilege operation, and
test coverage of the cloud edges --- to take a validated concept from prototype to production.