...
second provider (Section~\ref{sec:case-scenario}). Deploying it to GCP requires no change to
...
its ARN or URL (\S\ref{sec:resolution-cascade}), so the deployed workflow calls the new model without any change, with or without
...
authorize real transactions is set out in Section~\ref{sec:concl-applicability}.
