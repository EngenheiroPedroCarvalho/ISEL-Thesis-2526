...
Lambda ARN under one deployment and to a Cloud Functions URL under the other, subject on GCP to the
limitation of \S\ref{subsec:store-internals}. The uniform
...
Section~\ref{sec:case-deployment} shows the integration's central claim in a setting where it
plausibly matters. The fraud team's frequent model updates are a different matter: a retrained \texttt{fraud-check} redeployed in place keeps its endpoint
