...
In Terraform, a state machine definition can interpolate
\texttt{\$\{aws\_lambda\_function.lambda.arn\}}, the \texttt{arn} attribute of the Lambda resource
named \texttt{lambda}~\cite{terraformSfnStateMachine}; such a reference also creates an implicit
dependency between the two resources, so the function is created before the workflow that names
