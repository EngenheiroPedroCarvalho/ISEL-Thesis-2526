...
This chapter establishes the technical foundation for the proposed integration. It begins with the two layers the work builds on --- Serverless Computing and Function-as-a-Service (FaaS), and then the workflow layer that orchestrates the resulting functions --- highlighting in each the form of vendor lock-in that motivates this work.

It then analyses the two base technologies this dissertation extends: \textbf{QuickFaaS}, which handles the portable deployment of compute units, and \textbf{OmniFlow}, which manages the portable definition of orchestration logic. Finally, Section~\ref{ch:related-works} reviews related research and existing industry tools, identifying the gap in unified workflow orchestration that this work aims to close.
...
\section{Workflows and Orchestration}
\label{sec:workflows}
...
This section reviews the tools, frameworks and methods surveyed in the QuickFaaS publication~\cite{rodrigues2022quickfaas} and in the OmniFlow reports~\cite{costa2023omniflow,silva2025advanced} and positions them relative to the objectives of this thesis.
The remainder of this section discusses each group and highlights why none of them, in isolation, provides the combined behaviour targeted by the OmniFlow-QuickFaaS integration.
