...
Function-as-a-Service (FaaS), and then the workflow layer that orchestrates the resulting functions
--- highlighting in each the form of vendor lock-in that motivates this work; and then the two base
technologies this dissertation extends, \textbf{QuickFaaS}, which handles the portable deployment
of compute units, and \textbf{OmniFlow}, which manages the portable definition of orchestration
logic. Section~\ref{ch:related-works} turns to the state of the art, reviewing related research and
...
future work. That remaining step --- a portable workflow definition whose function references are
turned into concrete endpoints as the workflow is deployed --- is what this dissertation
addresses.
...
the template rather than the name the function ends up carrying --- which is why the workflow names
