...
Although each deployment target in this dissertation is a \emph{single provider at a time} (\S\ref{sec:int_cha_int_uni_multi}), portability concerns remain relevant because developers still want to avoid hard-coding endpoints and to preserve the ability to migrate or re-deploy the same application in another provider with minimal changes.
