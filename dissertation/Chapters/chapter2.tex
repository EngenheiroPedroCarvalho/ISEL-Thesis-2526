...
This distinction becomes relevant later, where the registry store and resolver internals are
described (\S\ref{subsec:store-internals}): the
proposed Function Registry validates and discovers GCP endpoints through the Cloud Run API, so
first-generation functions are recognised separately, by their \texttt{cloudfunctions.net} URL, and
are not yet covered by that validation and discovery. That domain is a heuristic rather than a
guarantee: a function created through the Cloud Functions v2 API is served on \texttt{run.app} but
...
\S\ref{sec:int_cha_int_uni_multi}, \texttt{arn:\allowbreak aws:\allowbreak lambda:\allowbreak%
<region>:\allowbreak <account-id>:\allowbreak function:\allowbreak <name>}: a fixed prefix, the
service, then the region and the account in which the function was created, and finally the name
it was given there. Only the first fields are known when the template is written: the account
and the deployed name are not, and \texttt{MyFunction} is the resource's logical identifier inside
