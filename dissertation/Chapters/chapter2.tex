\chapter{Serverless Portability: Background and State of the Art}
\label{cha:background}

This chapter has two parts. Section~\ref{sec:background} establishes the technical foundation for
the proposed integration: first the two layers the work builds on, Serverless Computing and
Function-as-a-Service (FaaS), and then the workflow layer that orchestrates the resulting
functions, highlighting in each the form of vendor lock-in that motivates this work; and then the
two base technologies this dissertation extends, \textbf{QuickFaaS}, which handles the portable
deployment of compute units, and \textbf{OmniFlow}, which manages the portable definition of
orchestration logic. Section~\ref{ch:related-works} turns to the state of the art, reviewing related research and
existing industry tools and identifying the gap in unified workflow orchestration that this work
aims to close.

\section{Background}
\label{sec:background}

\subsection{Serverless Computing and FaaS}
...
differences, which is the problem QuickFaaS addresses with its uniform hook/template model. The
second is
...
conventions, and QuickFaaS standardises on the one AWS, GCP and Azure all accept
(\S\ref{sec:background_quickfaas}). Beyond portability, the statelessness and
...
\subsection{Workflows and Orchestration}
\label{sec:workflows}

A \textit{workflow} describes how a process is executed as a sequence of steps, typically representing a business or operational procedure. In cloud environments, workflows are commonly modeled as \textit{state machines}, where each state performs a unit of work, makes a decision or controls execution flow, while passing data between states~\cite{aws_stepfunctions_concepts}. This is the layer the previous section pointed to, where the control flow, data flow and error handling of a multi-function application live~\cite{sebsflow_arxiv}.

Workflow systems typically provide support for:
\begin{itemize}
	\item \textbf{Control-flow}, such as sequential execution, conditional branching, parallelism, retries and waiting states;
...
Major cloud providers offer managed services for this role, notably AWS Step Functions and Google Cloud Workflows~\cite{aws_stepfunctions_concepts,gcp_workflows_overview}. They provide production-grade reliability and close integration with their respective FaaS platforms, but the logic defining how tasks interact is bound to provider-specific orchestration languages and schemas. This creates a second layer of vendor lock-in, distinct from the code-level one described above: workflow definitions must often be rewritten when moving to a different workflow engine or provider. That limitation has motivated portable, vendor-neutral workflow specifications, which decouple workflow definitions from a specific execution backend and improve interoperability across platforms~\cite{serverlessWorkflowSpec}.

\subsection{QuickFaaS: Portable Function Deployment}
...
\subsubsection{Components Overview}
...
\subsubsection{Uniform Programming Model}
...
\subsubsection{FaaS Deployment Model: The ZIP Strategy}
...
On GCP specifically, this pipeline provisions a first-generation Cloud Function through the
\texttt{cloudfunctions.googleapis.com/v1} API, rather than through the Cloud Run Admin API that
serves the newer generation. Google has since renamed both: the first generation is now documented
as a \emph{Cloud Run function (1st gen)}, and what used to be called a ``second-generation'' Cloud
Function is simply a \emph{Cloud Run function}, that is, a Cloud Run service deployed from source
code~\cite{gcpFunctionsVersionComparison}. This dissertation keeps the generation names, because
they are what distinguishes the two APIs the code calls. The resulting function is reachable at a
URL of the form \texttt{https://\allowbreak <region>-\allowbreak <project-id>.\allowbreak cloudfunctions.\allowbreak net/\allowbreak <name>}, and its
invocation permissions and deployment status are managed through that same first-generation API.
The distinction matters to the proposed Function Registry, which recognises a first-generation
function by its \texttt{cloudfunctions.net} URL; its consequences are assessed in
\S\ref{subsec:store-internals}. That domain is a heuristic rather than a guarantee: a function created through the Cloud Functions v2 API is served on \texttt{run.app} but
keeps a \texttt{cloudfunctions.net} endpoint as well~\cite{gcpFunctionsVersionComparison}.


\subsubsection{QuickFaaS function deployment example}
...
\begin{enumerate}
	...
	Lastly, the location of the cloud-agnostic function file the developer wishes to deploy. 
	The remaining fields complete the description. Inside \texttt{function}, \texttt{location} is the
	region the function is created in, and \texttt{bucket} names the Cloud Storage bucket used to
	stage the packaged source before the function is provisioned. The two trailing fields are
	optional, and are left empty in this example: \texttt{dependenciesFile} points at a file whose
	contents are injected into the dependency block of the generated \texttt{pom.xml}, so a function
	that uses no third-party library needs none, and \texttt{configurationsFile} supplies the
	\texttt{function-configs.json} packaged with the function. Both fall back to an empty default when omitted.
...
\end{enumerate}
\subsection{OmniFlow: Portable Workflow Orchestration}
...
\begin{enumerate}
	\item \textbf{Domain Specific Language (DSL):} the developer-facing interface used to define workflows in Kotlin. OmniFlow relies on Kotlin’s \emph{type-safe builder} style to provide a structured and readable syntax for declaring workflow metadata and steps~\cite{type-safe_builders}.
...
\subsubsection{Workflow Entity Model}
...
A step cannot be, for example, both an execution and a conditional step. In the concrete DSL~\cite{silva2025advanced, costa2023omniflow}, all four are represented as steps that differ only in their role within the workflow.

The underlying implementation still associates each step with metadata such as a \texttt{name} and, internally, a context object that captures the details of the chosen step kind. For execution steps, this context includes parameters such as HTTP method, host and path or variable assignments; for the other categories, it describes the nested branches or bodies to be executed. This abstraction allows the rest of the dissertation to reason about workflows in terms of a small set of step categories while remaining compatible with the richer, concrete model implemented by OmniFlow and used for rendering provider-specific workflow definitions.


\subsubsection{Workflow Development Cycle}
The typical OmniFlow lifecycle runs the components above as a synchronous three-phase process:
the developer \emph{defines} the workflow with the DSL, OmniFlow \emph{renders} the resulting tree
into the provider's format, and then \emph{deploys} it, at which point the workflow becomes a
managed resource that the provider executes~\cite{costa2023omniflow,silva2025advanced}.
...
The first step, \texttt{first-call}, uses the \texttt{call} context to perform an HTTP request. In this example, the request is a \texttt{POST} to the endpoint identified by \texttt{host("example-api.com")} and \texttt{path("/v1/process")}. The request body is constructed from the input data using \texttt{body(variable(\allowbreak "args.data"))}, showing how OmniFlow allows payloads to be derived from previously defined variables. The response is stored in the workflow variable \texttt{firstResult} through \texttt{result(\allowbreak "firstResult")}, which makes the response accessible to subsequent steps.
...
OmniFlow's call step is itself agnostic to what kind of compute resource answers at that endpoint: on GCP, it invokes whatever HTTP host and path the developer supplies, whether that address belongs to a (first-generation) Cloud Function or a Cloud Run service; the DSL and the renderer have no notion of the distinction described in \S\ref{sec:background_quickfaas}. This becomes significant once function endpoints are resolved automatically rather than hard-coded by the developer (Chapter~\ref{ch:proposed_solution}), since an automatic resolution mechanism must itself account for a difference that OmniFlow, by design, never had to.

A practical limitation follows from this design: OmniFlow call steps require concrete invocation
targets (e.g., \texttt{host} and \texttt{path}) to be known at definition or deployment
time~\cite{silva2025advanced}, so the functions they call must be deployed first and their
endpoints copied into the definition. This is the manual synchronisation described in
\S\ref{sec:int_cha_int_uni_multi}, which the proposed solution removes.
...
This section reviews the tools, frameworks and methods surveyed in the QuickFaaS
publication~\cite{rodrigues2022quickfaas} and in the OmniFlow
reports~\cite{costa2023omniflow,silva2025advanced}, together with more recent work, and positions
them relative to the objectives of this thesis. They fall into four groups: tooling that automates
deployment through templates or Infrastructure-as-Code (IaC) specifications; portable execution layers
and code transformation to FaaS; methods that measure portability instead of providing it; and
workflow composition and orchestration frameworks. Each group is discussed in turn, showing why
none of them, in isolation, provides the combined behaviour targeted by the OmniFlow--QuickFaaS
integration.

\subsection{Infrastructure-as-Code and Deployment-Oriented Tooling}
...
\textbf{Pulumi}~\cite{pulumi} takes a different approach by expressing infrastructure using general-purpose programming languages. It also explores serverless-specific abstractions such as \textit{Magic Functions}~\cite{pulumiMagicFunctions} and higher-level constructs through the \textit{Cloud Framework}~\cite{pulumiCloudFramework} and the \texttt{@pulumi/cloud} package~\cite{pulumiCloudPkg}. These abstractions move toward a more uniform experience but practical portability remains constrained by provider support coverage and by the fact that many real-world triggers, permissions and operational configurations remain provider-specific.

\textbf{CODE}~\cite{ristov2024code} is a more recent framework in this family, addressing
\emph{federated FaaS}: running the same function across several regions of several providers.
Following a ``code once, deploy everywhere'' principle, its DSL places the function at the top of a
three-level hierarchy (function, then providers, then regions) so that one declaration covers
every target instead of being repeated per provider and per region, which the authors report as up
to 9.23$\times$ fewer lines of code than Terraform or the Serverless Framework. A unified storage
library backs this DSL: one command uploads a deployment package to, or copies it between, the
storage services of AWS and GCP, so a function can be deployed directly from whichever provider's
storage already holds it, with no provider-specific upload code. CODE therefore shares this
thesis's ambition for the \textit{function} half of the problem, on the same two providers, but it
deploys the functions a workflow invokes rather than the workflow itself: the authors list the
automatic deployment of workflows written in their own choreography language, on top of CODE, as
future work. That remaining step is what this dissertation addresses: a portable workflow
definition whose function references are turned into concrete endpoints as the workflow is
deployed.

These tools also resolve references between resources at deployment time, including the endpoint
through which a workflow calls a function. Each of them lets the workflow definition name the
function through an expression that the deployment engine replaces with the function's identifier
once it is known. In AWS SAM, the \texttt{DefinitionSubstitutions} property of a state machine maps
placeholders in its Amazon States Language definition to values such as
\texttt{!GetAtt MyFunction.Arn}, a CloudFormation expression that reads the \texttt{Arn} attribute
of the function resource \texttt{MyFunction} declared in the same template~\cite{awsSamStateMachine}.
The value that expression yields is the function's Amazon Resource Name (ARN) introduced in
\S\ref{sec:int_cha_int_uni_multi}. Only its first fields are known when the template is written:
the account and the deployed name are not, and \texttt{MyFunction} is the resource's logical identifier inside
the template rather than the name the function ends up carrying. That is why the workflow names
it through an expression instead of a literal string. The reference also fixes the order of the
deployment: CloudFormation reads a \texttt{!GetAtt} to another resource as an implicit dependency,
and so creates the function before the state machine that substitutes its
ARN~\cite{awsCloudFormationDependsOn}.
The Step Functions plugin for the Serverless Framework accepts \texttt{Fn::GetAtt: [hello, Arn]}
(the same intrinsic in its long form) as the \texttt{Resource} of a task, and resolves it to the
ARN of the function \texttt{hello} declared in the same service~\cite{serverlessStepFunctionsPlugin}.
In Terraform, the state machine's definition is a string into which the tool interpolates
expressions written as \texttt{\$\{\ldots\}}, evaluated when the configuration is applied:
\texttt{\$\{aws\_lambda\_function.lambda.arn\}} names, in that order, the resource type
\texttt{aws\_lambda\_function}, the label \texttt{lambda} given to one such resource in the same
configuration, and the \texttt{arn} attribute that resource exports~\cite{terraformSfnStateMachine}.
As with \texttt{MyFunction} above, \texttt{lambda} is the label the configuration uses internally
rather than the name the function carries in AWS, and the attribute only holds a value once the
function has been created. Such a reference also creates an implicit dependency between the two
resources, so the function is created before the workflow that names
it~\cite{terraformReferences}. Pulumi likewise records a dependency when one resource's output is
passed as another's input, and orders their creation accordingly~\cite{pulumiInputsOutputs}. In each
case the function is declared alongside the workflow and deployed with it, and its endpoint is
written into the workflow at deployment time, the same static binding performed by the resolution
cascade proposed in this dissertation and described in \S\ref{sec:resolution-cascade}.
They also update a function whose code changed, which this work does not; conversely, they
...
language, with its own references to its own functions. This category therefore addresses
\textit{deployment automation}, binding a workflow to its functions within one template, but not
for a workflow definition that can be deployed to more than one provider.

\subsection{Portable Execution Layers and Code Transformation to FaaS}
...
\subsection{Assessing Portability Instead of Enabling It}

A third group steps back from enabling portability to \textit{measuring} it. The
\textbf{SEAPORT} method~\cite{yussupov2020seaport} proposes an automated way to assess the
portability of serverless applications (or orchestration tools) against a target provider by
transforming deployment models into a canonical representation (CASE).
...
\subsection{Workflow Composition and Orchestration Frameworks}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|p{0.36\textwidth}|c|c|c|c|c|}
    \hline
...
The OmniFlow--QuickFaaS integration combines these capabilities, including the on-demand
deployment of a referenced function that is missing, while remaining aligned with provider-native
execution surfaces (managed workflows and managed FaaS), avoiding the need to introduce and operate
an entirely new workflow runtime layer.