...
On a registry miss, the caller can skip the AWS scan by supplying the fully-qualified
\texttt{"region/\allowbreak functionRef"} form, which resolves a single region directly; regions where the
...
measured in Section~\ref{sec:eval-real-resolvers}. The same single-read strategy is also implemented by a benchmark fixture,
\texttt{OptimizedEndpointResolver}, which the local-resolution benchmark suite uses as a pure,
provider-agnostic reference implementation, without the live validation the real resolvers
perform against a cloud API.
...
the Cloud Run inspector: a hit is now confirmed live on both providers, through
\texttt{GetFunction} on AWS, and a drifted or deleted function is handled the same way on each
