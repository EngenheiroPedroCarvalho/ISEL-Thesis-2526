...
\begin{itemize}
    ...
    writing one entry per discovered function. The
    \texttt{Function\allowbreak Registry\allowbreak Bootstrapper} obtains that listing through
    the \texttt{Cloud\allowbreak Functions\allowbreak Catalog} interface, implemented by
    \texttt{Cloud\allowbreak Run\allowbreak V2\allowbreak Rest\allowbreak Catalog} (GCP) and
    \texttt{Lambda\allowbreak Functions\allowbreak Catalog} (AWS); on
    ...
    \item \textbf{Drift handling:} a registry hit whose endpoint has changed is refreshed. An
    entry whose function is no longer where the entry places it is replaced if the function is
    found in another region; otherwise the entry is removed and the deployment aborts, as Level~1
    of the cascade prescribes (\S\ref{sec:resolution-cascade}).
\end{itemize}
...
\S\ref{sec:resolution-cascade} on the Cloud Run API, through
\texttt{Cloud\allowbreak Run\allowbreak V2\allowbreak Service\allowbreak Inspector}, which looks
a service up by its full name to validate a hit and by project, region and name during
discovery. There is one exception: a hit on a
...
locations API, queried through
\texttt{Cloud\allowbreak Run\allowbreak Locations\allowbreak V1\allowbreak Rest\allowbreak Client},
whereas AWS has no equivalent for Lambda, so a dedicated
\texttt{Aws\allowbreak Regions\allowbreak Lister} interface was added to play that role, implemented
by \texttt{Ec2Aws\allowbreak Regions\allowbreak Lister} on the EC2 \texttt{DescribeRegions} call; an optional preferred region, when supplied, is tried first in both the
...
are deployed. The messages each resolver exchanges with the registry and with its provider's
inspection API are shown, level by level and with GCP and AWS side by side, in the sequence
diagrams of Appendix~\ref{app:cascade-sequences}
(Figures~\ref{fig:resolution-cascade-level1}--\ref{fig:resolution-cascade-level3}).
...
exists; QuickFaaS then finds the function through the Cloud Functions API and updates it in place
with the descriptor's code, so in this case the cascade does overwrite an existing function.
Without a descriptor, deployment fails and reports the function as absent. The AWS path is not
