...
\texttt{Lambda\allowbreak Function\allowbreak Inspector} seam, and a permissions failure aborts rather than
being mistaken for an absence. \\
...
rediscovery and the miss-path scan. \\
On AWS, regions where the lookup is refused are handled as on the GCP path (\S\ref{subsec:validation-error-semantics}). What
distinguishes the two resolvers is only the provider API each one calls (the region discovery
above, and a Cloud Run lookup versus \texttt{GetFunction} for the per-region check) and the
GCP-only short-circuit noted above.
