...
The primary goal of this dissertation is to achieve a more unified development and deployment experience for serverless workflows on a single cloud provider by integrating two complementary tools: QuickFaaS and OmniFlow. Two research questions guide the work:
...
To answer them, and to bridge the gap between these systems and reduce manual coordination, this work aims to:
...
\begin{itemize}
  ...
  \item \textbf{Chapter 6} presents an illustrative case study --- real-time fraud detection for card payments --- that grounds the registry and resolution cascade in a concrete banking scenario, deployed to both AWS and GCP.
  \item \textbf{Chapter 7} evaluates the prototype and answers the research questions: it checks the decisions of the resolution cascade on both providers, and measures the resolution overhead and its scalability, the registry write path, and the packaging overhead of the AWS agnostic layer.
