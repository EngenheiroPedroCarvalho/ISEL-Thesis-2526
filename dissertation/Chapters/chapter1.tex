...
\begin{itemize}
	...
	procedure in \S\ref{subsec:quickfaas-example} shows what this involves, and the URL it produces
	must then be written into the \texttt{host} and \texttt{path} of every OmniFlow call step that
...
\begin{itemize}
	...
	deployed to another account, project or provider therefore has a different endpoint.
...
\begin{itemize}
	...
	deployment will look for it. Only
...
deployment target sends the developer back through the manual pre-deployment described above. The
effort therefore grows with the number of functions a workflow calls and with the
...
\begin{itemize}
	...
	should be read and recorded by the tooling, which removes the copying step.
...
\begin{enumerate}
	\item \textbf{Design a unified integration architecture:} define the components and interfaces required for OmniFlow to coordinate workflow deployment with function availability. This includes the introduction of logical function identifiers (\texttt{functionRef}), a registry of invocation metadata and an explicit mechanism (the \texttt{internalFunction} construct on the call step) that allows the developer to have QuickFaaS deploy a function that does not exist yet.
...
\begin{enumerate}
	...
	registry stores the invocation metadata of each internal function, and a three-level
	...
	\item \textbf{AWS Lambda support in QuickFaaS.} QuickFaaS gains an AWS provider, and OmniFlow
	the matching function deployer, which extends the unification to AWS.
...
\begin{itemize}
  ...
  \item \textbf{Chapter 6} evaluates the prototype and answers the two research questions: whether the resolution cascade takes the right decisions on both providers, and what resolution costs.
