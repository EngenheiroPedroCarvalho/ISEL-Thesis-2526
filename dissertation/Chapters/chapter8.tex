...
\begin{itemize}
  ...
  internal calls takes about one millisecond locally, growing linearly with the number of internal
  calls, while the registry is read once per deployment
  (Section~\ref{sec:eval-discussion}); the reference resolver, which does not print a progress line
  per call, does the same work in $55\,\mu$s. These figures exclude the provider calls that
