...
\begin{itemize}
  ...
  deployment (Section~\ref{sec:eval-discussion}). These figures exclude the provider calls that
  validation, discovery and deployment make, which are counted in
...
\begin{enumerate}
  ...
  Functions that QuickFaaS deploys are not covered by validation or discovery
  (Section~\ref{sec:concl-critical}).
...
setting would require hardening the registry and the IAM model, as proposed in
Section~\ref{sec:concl-future}. None of these
