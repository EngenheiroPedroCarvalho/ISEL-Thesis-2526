...
The work delivered two complementary halves: the unification of function and workflow deployment,
through the Function Registry and the deployment-time resolution cascade
(Chapters~\ref{ch:proposed_solution} and~\ref{cha:impl}), and the AWS Lambda provider added to
QuickFaaS, with the matching deployer in OmniFlow (Section~\ref{subsec:quickfaas-aws-provider}).
How far each meets the objectives set out in Chapter~\ref{cha:introduction} is assessed next.
...
\begin{itemize}
  ...
  regions (Section~\ref{sec:eval-correctness}), within the limits stated below for objectives 2 and 4.
  \item \textbf{RQ2 (overhead)}: small. In the production resolvers, resolving a workflow of 200
  internal calls against a registry of ten functions takes $29\,\mu$s on AWS and $81\,\mu$s on GCP,
  growing linearly with the number of internal calls, while the registry is read once per
  deployment (Section~\ref{sec:eval-discussion}).
\end{itemize}
...
\begin{enumerate}
  ...
  missing ones, without endpoints being copied by hand. On GCP, however, the cascade covers the
  functions QuickFaaS deploys only in part (Section~\ref{sec:concl-critical}).
  ...
  evidence is local only: the correctness checks simulate the provider, the benchmarks ran on a
  laptop rather than dedicated hardware, and end-to-end deployment times were not measured. The
  provider calls that validation and discovery make are counted from the code
  (Section~\ref{sec:eval-api-calls}) rather than timed.
\end{enumerate}
...
\begin{itemize}
  ...
  and has to be deployed separately (for example, with QuickFaaS directly). The exception is a
  first-generation Cloud Function on GCP whose binding is missing from the registry, which the
  cascade cannot find and so redeploys over the existing function (\S\ref{subsec:store-internals}).
  ...
  \item \textbf{Partial GCP coverage.} On GCP, validation and discovery cover Cloud Run services
  only, not the first-generation Cloud Functions that QuickFaaS deploys
  (\S\ref{subsec:store-internals}); the AWS path has no such gap.
...
\begin{enumerate}
  \item \textbf{Cover the functions QuickFaaS deploys on GCP} by moving its GCP provider to the
  Cloud Functions~v2 API. A second-generation function \emph{is} a Cloud Run service, so the
  validation, discovery and bootstrap already built on the Cloud Run API would cover it unchanged,
  giving the GCP path the same drift detection and rediscovery guarantees as the AWS path. It would
  also retire the URL heuristic that recognises first-generation functions
  (\S\ref{sec:background_quickfaas}). The cost falls on QuickFaaS: the request body is
  restructured, the generated \texttt{run.app} URL has to be read from the deployed function
  instead of composed from the project and region, invoker permissions move to the Cloud Run Admin
  API, and the storage trigger's template has to adopt the CloudEvents interface. A Cloud
  Functions~v1 client alongside the Cloud Run one would close the same gap without touching
  QuickFaaS, but would leave the two generations to be told apart at resolution time.
  ...
  \item \textbf{Harden the registry:} a pluggable, access-controlled backend (database or secret/parameter store)
  ...
  \item \textbf{Validate the cascade against live providers,} deploying workflows that contain
  internal calls to real AWS and GCP accounts at each level of the cascade, so that the correctness
  evidence of Section~\ref{sec:eval-correctness} rests on the providers' own responses rather than
  on test doubles.
...
The solution fits an organisation that deploys one orchestration to several accounts or providers,
as the case study of Chapter~\ref{cha:case-study} illustrates, or that exposes open-banking
functions under PSD2. What remains is evidence against the real providers and engineering maturity (registry
