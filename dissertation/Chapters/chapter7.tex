...
The absolute numbers characterise the machine they were measured on rather than serverless
deployment hardware in general, and the factor they are most sensitive to is storage rather than
processor speed (\S\ref{sec:eval-methodology}). What they support is therefore the shape of each
...
A second limit is one of provenance: T6 and T7 come from a second measurement session while every
other experiment comes from the first (\S\ref{sec:eval-methodology}). The two sessions agree on
the quantity they share, but the comparison between T6, T7 and the reference path of T4 crosses
that boundary and should be read as an order-of-magnitude comparison rather than an exact one.
