...
names the same function ($F{=}1$). T6 and T7 do not include these requests, since they replace
the lookup with a local fake on AWS and skip it on GCP (\S\ref{sec:eval-real-resolvers}); with a
real provider, each adds a network round-trip per internal call. The list of candidate regions, by contrast, is
...
the scope of this evaluation (\S\ref{sec:eval-scope}).
...
The correctness evidence for RQ1 is local as well. The resolver tests replace the provider with
