...
JIT compiler. Every figure reported in this chapter comes from one suite run, executed in a single
session with three JVM forks, three warm-up iterations and five measurement iterations of one
second each (\texttt{-f 3 -wi 3 -i 5 -w 1 -r 1}); each value is therefore the mean of fifteen
measured iterations across three JVMs, of already-JIT-compiled code. Values from different
experiments are directly comparable, since they share the run.
...
The result has the same shape as the already-optimized reference implementation --- cost dominated
by $N$, growing only mildly with $F$ --- but the per-call constant is an order of magnitude larger:
about $4.7\,\mu$s per call on AWS and $7.1\,\mu$s on GCP, against the $0.22\,\mu$s that P8's
optimized path pays for the same snapshot lookup in the same run. The extra work is not the
registry read, which happens once, but what each production resolver does per call: the validation
step, and a progress line printed to the console.

That console output is the larger part, and the gap between the two providers is what shows it. On
the measured path the AWS resolver prints two lines per call and the GCP resolver three, and the
measured difference --- $2.4\,\mu$s per call --- is almost exactly one line. Attributing about
$2.3\,\mu$s to each printed line leaves roughly the $0.2\,\mu$s of resolution work that P3
measures, which accounts for both figures. P10 and P11 therefore quantify the production resolvers
\emph{as they behave today}, console output included; they are not a measurement of resolution
logic alone, and the difference between the providers is a difference in logging, not in the
cascade (\S\ref{sec:eval-threats}).
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrrr}
    ...
    $F\backslash N$ & 1 & 5 & 10 & 50 & 200 \\
    \hline
    1  & 16.6 & 34.0 & 53.6 & 238 & 955    \\
    2  & 18.6 & 36.5 & 56.1 & 240 & 966    \\
    5  & 20.8 & 38.0 & 55.9 & 250 & 992    \\
    10 & 20.7 & 39.6 & 63.5 & 259 & 1\,006 \\
    20 & 22.9 & 43.6 & 66.1 & 262 & 1\,018 \\
    50 & 28.6 & 48.5 & 72.6 & 259 & 1\,022 \\
    \hline
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrrr}
    ...
    $F\backslash N$ & 1 & 5 & 10 & 50 & 200 \\
    \hline
    1  & 20.7 & 49.7 & 74.9 & 364 & 1\,439 \\
    2  & 20.9 & 48.9 & 74.5 & 360 & 1\,445 \\
    5  & 21.5 & 49.6 & 77.4 & 354 & 1\,376 \\
    10 & 20.5 & 46.1 & 74.9 & 335 & 1\,344 \\
    20 & 23.8 & 50.4 & 80.4 & 344 & 1\,400 \\
    50 & 29.8 & 58.0 & 86.7 & 345 & 1\,377 \\
    \hline
  \end{tabular}
\end{table}
...
now show the same dominated-by-$N$ shape as the optimized reference path rather than the
call-times-registry-size cost they used to pay; their per-call constant is an order of magnitude
larger, but that difference is the progress line each resolver prints per call, not resolution
work.
...
A second limit is that the production resolvers were measured as they behave today, printing a
progress line per resolved call. In P10 and P11 that console output accounts for most of the
per-call cost --- around $2.3\,\mu$s per printed line, against $0.22\,\mu$s of resolution work ---
so those two experiments should be read as the cost of the resolvers in their current form, not as
the cost of resolution. Silencing the output, or routing it through the logger already present in
both classes, would bring them close to the reference path of P8.
