...
semantics prescribe (\S\ref{sec:walkthrough} and \S\ref{subsec:validation-error-semantics}). The observed behaviour matches the
...
The tests check the decisions of the cascade, not the behaviour of the providers
...
other experiment comes from the first (\S\ref{sec:eval-methodology}), so the comparison between T6, T7 and the reference path of T4 crosses
