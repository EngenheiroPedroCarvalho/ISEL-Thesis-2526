...
names the same function ($F{=}1$), each a network round-trip that T6 and T7 do not include. The list of candidate regions, by contrast, is
...
lookups above but by the deployment sequence. On AWS the deployer resolves the execution role
...
per-call reads they used to pay for: $29\,\mu$s (AWS) and $81\,\mu$s (GCP) for a
200-call workflow.
...
actual cloud deployment (seconds). These figures are also a lower bound: the network
round-trips that validation and discovery add are counted in \S\ref{sec:eval-api-calls}, not timed
(\S\ref{sec:eval-scope}).
