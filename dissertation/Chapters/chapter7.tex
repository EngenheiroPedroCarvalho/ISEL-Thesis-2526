...
\S\ref{subsec:store-internals}, whose effect T3--T5 demonstrated on the reference resolver.
...
optimized reference path: $29\,\mu$s (AWS) and $81\,\mu$s (GCP) for a 200-call workflow.
...
(\S\ref{sec:eval-methodology}) rather than serverless deployment hardware in general. File I/O
...
A second limit is one of provenance: because T6 and T7 come from a second measurement session
(\S\ref{sec:eval-methodology}), their comparison with the reference path of T4 should be read as
