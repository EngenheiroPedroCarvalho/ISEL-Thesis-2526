...
naive path (Table~\ref{tab:eval-t4-optimized}; both paths are plotted in
Figure~\ref{fig:eval-t4}): at $F{=}1$ it goes from $10.2$ to $54.7\,\mu$s as $N$ goes from 1 to
200, where the naive path goes from $10.4$ to $2\,069\,\mu$s, while at $N{=}1$ it goes only from
...
\begin{figure}[htbp]
  ...
  $R{=}F$. On both paths the curves are ordered by $F$ and climb with $N$, steeply on the naive
  path and gently on the optimized one.}
  \label{fig:eval-t4}}
\end{figure}
...
the write rewrites it. The production resolvers do not pay this surcharge: they look every call up
in the snapshot with \texttt{tryResolveEntryIn}, so their writes cost what T8 measures. The surcharge grows with $R_0$ (about 30\% on an empty registry, about
...
\begin{figure}[htbp]
  ...
  size $R_0$ (both axes logarithmic). The curves are nearly parallel straight lines, with slopes
  from $1.06$ ($R_0{=}0$) down to $0.93$ ($R_0{=}1000$): the total is close to linear in $K$, and
  $R_0$ mainly shifts it.}
  \label{fig:eval-t8}}
\end{figure}
...
to $I{=}200$ multiplies the cost by five (Table~\ref{tab:eval-t11} and Figure~\ref{fig:eval-t11}). Each row is flat apart from
...
\begin{figure}[htbp]
  ...
  number of internal calls $I$. Each curve is flat in the number of external calls $E$ apart
  from isolated jumps that vanish at the next point, and the curves separate by $I$ alone.}
  \label{fig:eval-t11}}
\end{figure}
...
\begin{figure}[htbp]
  ...
  fixed (logarithmic vertical axis). The depth-2 spike carries a confidence interval of
  $\pm40.8\,\mu$s, more than half its mean, which marks it as noise.}
  \label{fig:eval-t13}}
\end{figure}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|p{0.30\textwidth}|p{0.30\textwidth}|p{0.30\textwidth}|}
    ...
    \hline
    Level~1, first-generation Cloud Function in the registry & n/a & 0 (validation skipped,
...
cost of one registry read (\S\ref{sec:eval-methodology}). The comparison between T6, T7
and the reference path of T4 nonetheless crosses that boundary and should be read as an
order-of-magnitude comparison rather than an exact one.
