...
about $9.8\,\mu$s (Table~\ref{tab:eval-t1}). What separates the two columns is the per-call work.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{images/T2_registry_scaling.png}
...
than with $N$ (Table~\ref{tab:eval-t4-optimized}): at $F{=}1$ it goes from $10.2$ to
...
$46\times$ at $R{=}1000$ (Table~\ref{tab:eval-t5}). The optimized column is the cost of one read
...
registry (Table~\ref{tab:eval-t10}).
...
to $I{=}200$ multiplies the cost by five (Table~\ref{tab:eval-t11}). Each row is flat and each
...
$199\,\mu$s at $R{=}1000$ (Table~\ref{tab:eval-t12}).
...
(Tables~\ref{tab:eval-t13} and~\ref{tab:eval-t14}). Although the resolver recurses explicitly into
