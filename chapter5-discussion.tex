...
file from disk. P6 isolated the second dimension and characterised the cost as
$\Theta(N\cdot R)$ (with $N$ calls over an $R$-entry registry): about
$180\,\mu$s of fixed I/O and parsing per read plus roughly
$0.56\,\mu$s per registered entry. P7 then applied the obvious remedy --- read the
registry once per workflow into an in-memory snapshot and resolve against it --- turning
$\Theta(N\cdot R)$ into $\Theta(N+R)$ and delivering up to a \(\sim\!200\times\) speedup in the
...
measure exactly these classes and confirm they still pay the full $\Theta(N\cdot R)$ cost per
deployment, because each call goes through \texttt{FunctionRegistryStore.tryResolveEntry}, which
re-reads the whole file. In other words, the headline \(\sim\!200\times\) speedup is a property
...
$\Theta(K\cdot R_0 + K^2)$ (P13), and the miss-then-write pattern the real resolvers use before
...
(P3, P6), the $\Theta(N\cdot R)$ cost was quantified, and the single-read optimisation
...
\begin{enumerate}
    ...
    measured $\Theta(N+R)$ optimum and the $\Theta(N\cdot R)$ cost these classes still pay
