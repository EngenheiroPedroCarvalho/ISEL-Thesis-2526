...
internal class AmazonRendererTest {
    ...
    @Test
    fun `test lambda call step uses Payload result`() {
    ...
    @Test
    fun `test api gateway call step uses ResponseBody result`() {
        val w = createWorkflow(
            step {
                name("CallApi")
        ...
        expectThat(content).and {
            get { contains("arn:aws:states:::apigateway:invoke") }.isEqualTo(true)
            get { contains("\$.ResponseBody") }.isEqualTo(true)
            get { contains("\$.Payload") }.isEqualTo(false)
        }
    }
