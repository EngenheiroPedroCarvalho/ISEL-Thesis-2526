 ...
 * isolate the effect of the REGISTRY SIZE (M) on resolution, holding the workflow
 * fixed. The workflow always makes [FIXED_CALLS] calls over [FIXED_FUNCTIONS]
 * distinct functions (N and F fixed); only the registry is padded to M entries
 * (M >= [FIXED_FUNCTIONS]), so the workflow references just the first F of them
 * while M-F extra entries inflate every registry read.
 *
 * Same two strategies as P8, resolution only (no render, which the read
 * optimization does not affect):
 *  - [resolveNaive]     re-reads the whole M-entry registry file per call -> O(N*M)
 *  - [resolveOptimized] reads the M-entry registry once, then N lookups    -> O(N+M)
 *
...
open class BenchmarkResolveWorkflowByRegistrySize {
    ...
    /** Number of functions registered in the registry file (M); always >= FIXED_FUNCTIONS. */
    ...
    fun setupWorkflow() {
        ...
        // Registry padded to M entries; the workflow references only the first F.
        val functions = (0 until m).associate { idx ->
            val name = "$BASE$idx"
