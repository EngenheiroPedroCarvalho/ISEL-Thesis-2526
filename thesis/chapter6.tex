...
\begin{enumerate}
  ...
  resolution overhead ($\Theta(N\cdot R)$, reducible to $\Theta(N+R)$), the write-path cost, and
...
\begin{itemize}
  ...
  to $\Theta(N+R)$ lives in the superseded resolver, not in the production
  \texttt{AwsInternalFunctionResolver} and \texttt{WorkflowInternalFunctionResolver}, which still
  pay the full $\Theta(N\cdot R)$ cost (Section~\ref{sec:eval-real-resolvers}).
  ...
  build systems across a process boundary and keying error handling off child-process text.
...
\begin{enumerate}
  ...
  gap between the measured $\Theta(N+R)$ optimum and the $\Theta(N\cdot R)$ cost these classes
