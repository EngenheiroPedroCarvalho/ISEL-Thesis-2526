...
\begin{figure}[hbt!]
	\centering
	\includegraphics[width=0.8\textwidth]{images/QuickFaaS_Test/FaaS_Deployment.png}
	\caption{FaaS deployment to GCP. Adapted from~\cite{rodrigues2022quickfaas}}
	\label{fig:function_deployment}
\end{figure}

On GCP specifically, this pipeline provisions a (first-generation) Cloud Function through the \texttt{cloudfunctions.googleapis.com/v1} API, rather than a Cloud Run service --- the platform's newer, container-based serverless product, also marketed as ``second-generation'' Cloud Functions. The resulting function is reachable at a URL of the form \texttt{https://<region>-<project>.cloudfunctions.net/<name>}, and its invocation permissions and deployment status are managed through that same first-generation API. This distinction becomes relevant later in the dissertation (\S\ref{subsec:store-internals}), where the proposed Function Registry has to recognise first-generation Cloud Functions separately from Cloud Run services when validating and discovering endpoints.
...
Figure~\ref{fig:omniflow_components} summarizes these components.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{./images/Omniflow/omniflow_components.png}
	\caption{OmniFlow component diagram illustrating internal modules and external interactions. Adapted from~\cite{costa2023omniflow,silva2025advanced}.}
	\label{fig:omniflow_components}
\end{figure}
...
OmniFlow models a workflow as a root \textbf{Workflow} entity that is \emph{composed by} one or more steps, as illustrated in the abstract model of Figure~\ref{fig:abstract-workflow-definition}. At the DSL level, the workflow root typically includes a \texttt{name}, a \texttt{description}, an input variable and a result variable~\cite{silva2025advanced}. In the ER view, these are captured by two explicit entities: input, representing the data received by the workflow, and output, representing the value returned at the end of execution. A workflow therefore \emph{receives} exactly one input and \emph{returns} exactly one output in each execution.

\begin{figure}[H]
  \centering
  % adjust the path/width as needed
  \includegraphics[width=0.95\textwidth]{images/Omniflow/abstract-definition-workflow.png}
  \caption{Abstract workflow entity model. Adapted from~\cite{silva2025advanced}}
  \label{fig:abstract-workflow-definition}
\end{figure}


Each \textbf{Step} represents a unit of behaviour inside the workflow. The abstract model makes explicit that a step can belong to one of four disjoint categories:
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.95\textwidth]{images/Omniflow/omniFlow_sequence_diagram.png}
	\caption{OmniFlow sequence diagram illustrating interactions between the developer, the library and the cloud provider. Adapted from~\cite{costa2023omniflow}.}
	\label{fig:omniflow_sequence}
\end{figure}
...
Overall, this category primarily addresses \textit{deployment automation}. It helps reduce manual work but it does not fully solve the problem targeted by this thesis: keeping workflows stable and provider-agnostic while automatically ensuring that the referenced functions exist and can be resolved at deployment time.
...
Table~\ref{tab:related-comparison} summarises the surveyed families against the five dimensions
that matter for this thesis. Two observations stand out. First, the columns are rarely satisfied
...
execute the workflow on their own runtime instead of a provider-native service. Second, and most
importantly, no surveyed work provides the last column --- resolving a workflow's logical function
references into concrete endpoints \emph{at workflow-deployment time}, deploying the missing
functions on demand. That column is precisely the gap the OmniFlow--QuickFaaS integration fills
while keeping both the workflow and the functions on provider-native surfaces.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccccc}
    \hline
    \textbf{Approach} & \textbf{Deploy.} & \textbf{Func.} & \textbf{Workflow} & \textbf{Workflow-first} & \textbf{Provider-} \\
     & \textbf{autom.} & \textbf{portab.} & \textbf{portab.} & \textbf{resolution} & \textbf{native} \\
    \hline
    IaC (CloudFormation, Terraform, Serverless, Pulumi) & Y & P & -- & -- & Y \\
    ...
    \textbf{OmniFlow + QuickFaaS (this work)}           & Y & Y & Y & Y & Y \\
    \hline
  \end{tabular}
\end{table}
