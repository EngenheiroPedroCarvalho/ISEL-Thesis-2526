...
two panels make the asymmetry noted above explicit --- on a \texttt{NotFound} result, the GCP path
(a) attempts one more rediscovery across regions before removing the stale entry, while the AWS
path (b) removes it directly, since a Lambda name is already unique within its region.

Figure~\ref{fig:resolution-cascade-level2} (Appendix~\ref{app:cascade-sequences}) details Level~2,
reached on a registry miss. The GCP resolver (a) searches across the project's regions (or a
single region when the fully-qualified \texttt{"region/functionRef"} form is used), reporting more
than one match as ambiguous; the AWS resolver (b) performs a single \texttt{GetFunction} call and,
on this path only, does not distinguish a \texttt{NotFound} result from a permissions failure,
both being treated as a miss (\S\ref{subsec:validation-error-semantics}).
