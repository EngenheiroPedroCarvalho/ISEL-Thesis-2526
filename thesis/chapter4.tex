...
\texttt{"region/functionRef"}, raising if the suffix match is ambiguous) by re-reading the file and
delegating to its pure counterpart, \texttt{tryResolveEntryIn()}, which runs the same two-step
lookup against an already-loaded map with no I/O of its own --- the same split \texttt{resolveUrl()}/
\texttt{resolveUrlIn()} already had; \texttt{put()} inserts or refreshes one entry; and
