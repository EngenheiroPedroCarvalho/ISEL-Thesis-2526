...
ambiguous. \texttt{AwsInternalFunctionResolver} is simpler because a Lambda is uniquely named
within a region: a registry miss triggers a single \texttt{GetFunction} call, and only a genuine
\texttt{ResourceNotFoundException} (rather than any error) is treated as ``not present''. In both
