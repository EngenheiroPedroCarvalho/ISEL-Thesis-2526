...
which reflects the two clouds' invocation models. \texttt{WorkflowInternalFunctionResolver} (GCP)
...
ambiguous. \texttt{AwsInternalFunctionResolver} treats a registry hit as final rather than
...
\texttt{WorkflowInternalCallEndpointResolver} reads the registry once into a snapshot and resolves
...
\texttt{FunctionEndpointKeys} and consumed by \texttt{WorkflowInternalCallEndpointResolver}. This
...
\emph{inside} QuickFaaS (Part~A), which performs the actual Lambda deployment, and the
\texttt{AwsLambdaDeployer} added \emph{inside} OmniFlow (Part~B), which prepares the environment,
...
carries the \texttt{lambda://} prefix (the marker written by \texttt{AwsInternalFunctionResolver}),
