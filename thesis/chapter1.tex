...
\begin{itemize}
	\item \textbf{Establish a unified deployment lifecycle:} enable OmniFlow to deploy workflows while also coordinating function availability through QuickFaaS when the developer explicitly requests it (e.g., by declaring an internal call via the \texttt{internalFunction} construct).
...
\begin{itemize}
  ...
  \item \textbf{Chapter 5} describes the implementation of the integration: the function registry and its deployment-time resolution cascade, the AWS Lambda support added to QuickFaaS, and the runtime invocation of internal functions.
