...
\begin{itemize}
  ...
  (\texttt{AwsInternalFunctionResolver}, \texttt{WorkflowInternalFunctionResolver}) and of the
  registry write path.
...
(Table~\ref{tab:eval-p3}). In the general case the cost is $\Theta(N\cdot R)$.
...
single line, confirming $\Theta(N\cdot R)$ where the fixed cost of reopening the file dominates for
...
Reading the registry once per workflow into an in-memory snapshot and resolving against it turns
$\Theta(N\cdot R)$ into $\Theta(N+R)$. Table~\ref{tab:eval-p7} compares, in the same run, resolving
...
worst corner ($N{=}200$, $R{=}1000$: $\sim$105\,ms down to $\sim$0.54\,ms). Experiment P9 confirms
the same trend on a realistic workflow ($F{=}10$, $N{=}50$ fixed), where the speedup grows from
$\sim$41$\times$ at $R{=}10$ to $\sim$58$\times$ at $R{=}1000$.
...
Experiments P10 and P11 measure the resolvers that actually run during deployment,
\texttt{AwsInternalFunctionResolver} and \texttt{WorkflowInternalFunctionResolver}, with the
registry always hitting ($R{=}F$). Both are dominated by $N$ and are almost indifferent to $F$
($\sim$355\,$\mu$s/call on AWS; $\sim$7--8\% more on GCP due to the extra provider-specific glue,
not the shared registry mechanism). Crucially, these classes still pay the full $\Theta(N\cdot R)$
cost, because each call goes through \texttt{FunctionRegistryStore.tryResolveEntry}, which re-reads
the whole file: the single-read optimization demonstrated in P7 has not yet been applied to them.
This is a documented, quantified optimization opportunity rather than an applied change.
Table~\ref{tab:eval-p10p11} reports both resolvers over the same $F\times N$ grid; the per-call cost
is flat in $F$ and rises linearly with $N$, and the GCP resolver is consistently a few percent
dearer than the AWS one.

\begin{table}[htbp]
  \centering
  \caption{P10/P11 --- resolution time ($\mu$s) of the real AWS and GCP resolvers, $R{=}F$.}
  \label{tab:eval-p10p11}
  \begin{tabular}{lrrrrr}
    \hline
    $F\backslash N$ & 1 & 5 & 10 & 50 & 200 \\
    \hline
    \multicolumn{6}{l}{\emph{AWS --- \texttt{AwsInternalFunctionResolver} (P10)}} \\
    1  & 356 & 1\,684 & 3\,338 & 17\,806 & 71\,825 \\
    10 & 355 & 1\,738 & 3\,421 & 17\,509 & 69\,700 \\
    50 & 383 & 1\,847 & 3\,732 & 22\,396 & 73\,603 \\
    \multicolumn{6}{l}{\emph{GCP --- \texttt{WorkflowInternalFunctionResolver} (P11)}} \\
...
registry file, so writing $K$ functions into a registry that already holds $R_0$ entries costs
$\Theta(K\cdot R_0 + K^2)$ (P13). The miss-then-write pattern the real resolvers perform before any
cloud call (\texttt{tryResolveEntry} followed by \texttt{put}) adds a further 24--54\% on top
(P17), since \texttt{tryResolveEntry} is itself $\Theta(R)$. In absolute terms these values remain
in the hundreds of microseconds to low hundreds of milliseconds even for $K{=}100$ successive
writes (Table~\ref{tab:eval-p13p17}).

\begin{table}[htbp]
  \centering
  \caption{P13/P17 --- total write time ($\mu$s): $K$ successive \texttt{put} (P13) and
  $K$ miss+\texttt{put} pairs (P17), initial registry size $R_0$.}
  \label{tab:eval-p13p17}
  \begin{tabular}{lrrrrr}
    \hline
    $K\backslash R_0$ & 0 & 10 & 50 & 200 & 1000 \\
    \hline
    \multicolumn{6}{l}{\emph{P13 --- \texttt{put} only}} \\
    1   & 2\,673   & 2\,860   & 3\,235   & 4\,139   & 2\,761   \\
    10  & 27\,691  & 26\,986  & 29\,912  & 39\,164  & 24\,029  \\
    100 & 329\,757 & 304\,187 & 330\,100 & 419\,531 & 237\,910 \\
    \multicolumn{6}{l}{\emph{P17 --- miss + \texttt{put}}} \\
...
reintroducing an $\Theta(N\cdot R)$ term, but over an already-loaded map: the ratio to exact match
grows to only $\sim$1.9$\times$ at $R{=}1000$ and remains cheap in absolute terms
...
because external calls incur no registry lookup (Table~\ref{tab:eval-p15}). The $\Theta(R)$
behaviour proved in P9 is unchanged when a fraction of the calls are external (P16).
...
(Table~\ref{tab:eval-p18p19}). Although the resolver recurses explicitly into \texttt{Iteration}
and \texttt{Parallel} contexts, the shape of the tree is irrelevant: only the count of internal
calls matters.

\begin{table}[htbp]
  \centering
  \caption{P18/P19 --- resolution time ($\mu$s) vs.\ nesting depth (20 calls fixed) and vs.\
  parallel width ($N = 5\times$ width), $R{=}F{=}10$.}
  \label{tab:eval-p18p19}
  \begin{tabular}{rr@{\qquad}rrr}
    \hline
    \multicolumn{2}{l}{\emph{P18 --- depth}} & \multicolumn{3}{l}{\emph{P19 --- parallel width}} \\
    \textbf{depth} & \textbf{time} & \textbf{branches} & \textbf{$N$} & \textbf{time} \\
    \hline
    0 & 207.3 & 1   & 5   & 188.3 \\
    2 & 200.7 & 10  & 50  & 220.8 \\
    3 & 200.4 & 20  & 100 & 276.2 \\
    5 & 199.6 & 100 & 500 & 583.7 \\
    \hline
  \end{tabular}
\end{table}
...
characterised: $\Theta(N\cdot R)$ in the naive form, dominated by the fixed cost of reopening the
registry file until $R \approx 320$. Second, the single-read optimization eliminates the $N\cdot R$
product, reducing resolution to $\Theta(N+R)$ with a $\sim$200$\times$ speedup in the worst case
(P7, P9) --- a full \emph{identify (P3/P6) $\rightarrow$ fix $\rightarrow$ quantify (P7)} loop.
Third, this optimization has not yet been propagated to the production resolvers (P10, P11), which
still pay the full cost; doing so is a small, well-scoped change and is listed as future work.
Fourth, the AWS agnostic packaging layer adds negligible size (S1).
...
deployment: the dominant cost is the actual cloud deployment (seconds). Once the single-read
optimization is applied uniformly, the overhead of the unification is asymptotically linear and
practically irrelevant in real operation.
