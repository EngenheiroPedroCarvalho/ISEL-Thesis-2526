...
because external calls incur no registry lookup. The $\Theta(R)$ behaviour proved in P9 is
unchanged when a fraction of the calls are external (P16).
...
Although the resolver recurses explicitly into \texttt{Iteration} and \texttt{Parallel} contexts,
the shape of the tree is irrelevant: only the count of internal calls matters.
