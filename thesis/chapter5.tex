...
provider: \texttt{AwsInternalFunctionResolver} (Table~\ref{tab:eval-p10}) and
\texttt{WorkflowInternalFunctionResolver} (Table~\ref{tab:eval-p11}), with the
...
not the shared registry mechanism). Crucially, these classes still pay the full cost, growing in
proportion to both the number of calls and the registry size,
because each call goes through \texttt{FunctionRegistryStore.tryResolveEntry}, which re-reads
...
In both tables the per-call cost is flat in $F$ and rises linearly with $N$, and the GCP resolver
(Table~\ref{tab:eval-p11}) is consistently a few percent dearer than the AWS one
(Table~\ref{tab:eval-p10}).
