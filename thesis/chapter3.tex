...
been deployed and the registry does not yet exist. OmniFlow first bootstraps the registry (on GCP,
by listing the project's HTTP-invocable services; on AWS it simply starts empty) and then walks the
workflow tree. The \texttt{external-risk-api} step carries literal \texttt{host}/\texttt{path} and
